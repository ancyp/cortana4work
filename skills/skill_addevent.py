...
contacts = {'ilya': 'ilya.zilbersteine@microsoft.com', 'ancy': 'philip.ancy@microsoft.com',
            'pavan': 'pavan.kemparaju@microsoft.com', 'wellington': 'wduraes@microsoft.com'}
    ...
    if post_data['intent'] == 'add-task' or post_data['intent'] == 'add-meeting':
        ...
        for e in post_data['entities']:
            ...
            if 'builtin.datetimeV2.duration' in e:
                duration = int(
                    ''.join(filter(str.isdigit, e["builtin.datetimeV2.duration"])))
            if "builtin.datetimeV2.date" in e:
                if 'min' in e["builtin.datetimeV2.date"]:
                    duration = int(
                        ''.join(filter(str.isdigit, e["builtin.datetimeV2.date"])))
                else:
                    for month in months:
                        if month in e["builtin.datetimeV2.date"]:
                            deadline = datetime(2018, months[month], int(
                                ''.join(filter(str.isdigit, e["builtin.datetimeV2.date"]))), 17, 00)
        if subject and deadline and duration:
            ...
            if post_data['intent'] == 'add-meeting':
                event['attendees'] = [{'emailAddress': {
                    'address': contacts[participant], 'name': participant}, 'type': 'required'}]
            r = make_api_call('POST', get_messages_url, token, payload=event)
            if (r.status_code == requests.codes.ok or r.status_code == 201):
                resp = Response(json.dumps(
                    {"txt-response": "Task successfully added at {}".format(time)}), status=200, mimetype='application/json')
                return "<div> Task successfully added at {} </div>".format(time)
                # return resp
            else:
                print("{0}: {1}".format(r.status_code, r.text))
    resp = Response(json.dumps(
        {"txt-response": "Failed to add task."}), status=200, mimetype='application/json')
    return "<div> Failed to add task </div>"
    # return resp


def find_open_time(deadline, duration):
    date_now = (datetime.now() + timedelta(minutes=60)
                ).replace(microsecond=0, second=0, minute=0)
    # print((deadline - date_now).days)
    all_events = get_events(date_now, (deadline - date_now).days)
    listings = [True] * (((deadline - date_now).days * 48) +
                         (deadline - date_now).seconds // 60 // 30)
