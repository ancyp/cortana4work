...
def api_root():
  ...
  if post_data['intent'] == 'add-task':
            ...
            if 'min' in e["builtin.datetimeV2.date"]:
                ...
                for month in months:
                    if month in e["builtin.datetimeV2.date"]:
                        start = datetime(2018, months[month], int(''.join(filter(str.isdigit, e["builtin.datetimeV2.date"]))))
      if subject and start and duration:
          ...
          if (r.status_code == requests.codes.ok):
            resp = Response(json.dumps({"txt-response": "Task successfully added!"}), status=200, mimetype='application/json')
            return resp
          else:
